...
var ComponentPie = function ( name, cfg ) {
    ...
    for (var i = 0; i < step; i++) {
        ...
        // text.css('left',x/2); text.css('top',y/2);
        ...
        if (cfg.data[i][2]) {
            ...
            // text.css('color','#fff'); text.css('backgroundColor',cfg.data[i][2]);
        }
        text.css('opacity', 0);
...
ComponentPie.reSort = function (list) {
    ...
    var compare = function (domA, domB) {
        ...
        //  domA 的投影
        var shadowA_x = [
            offsetA.left, $(domA).width() + offsetA.left
        ];
        var shadowA_y = [
            offsetA.top, $(domA).height() + offsetA.top
        ];

        //  domB 的投影
        var shadowB_x = [
            offsetB.left, $(domB).width() + offsetB.left
        ];
        var shadowB_y = [
            offsetB.top, $(domB).height() + offsetB.top
        ];
