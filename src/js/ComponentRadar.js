/* 雷达图组件对象 */

var ComponentRadar =function ( name, cfg ) {
    var component =  new PublicComponent( name ,cfg );
    
    //  绘制网格线 - 背景层
    var w = cfg.width;
    var h = cfg.height;
  
    //  加入一个画布（网格线背景）
    var canvasBg = document.createElement( "canvas" );
    var contextBg = canvasBg.getContext('2d');
    canvasBg.width  = contextBg.width  = w;
    canvasBg.height = contextBg.height = h;
    component.append( canvasBg );
  
    var r = w / 2;
    var step = cfg.data.length;
  
  
    //  计算一个圆周上的坐标（计算多边形的顶点坐标）
    //  已知：圆心坐标(a, b)、半径 r；角度 deg。
    //  rad = ( 2 * Math.PI / 360 ) * ( 360 / step ) * i
    //  x = a + Math.sin( rad ) * r;
    //  y = b + Math.cos( rad ) * r;
  
    //  绘制网格背景（分面绘制，分为10份）
    var isBlue = false;
    for( var s = 10; s > 0; s -- ) {
    
        contextBg.beginPath();
        for( var i = 0; i < step; i ++ ) {
            var  rad = ( 2 * Math.PI / 360 ) * ( 360 / step ) * i;
    
            var x = r + Math.sin( rad ) * r * ( s / 10 );
            var y = r + Math.cos( rad ) * r * ( s / 10 );
            contextBg.lineTo( x, y );
        }
        contextBg.closePath();
        contextBg.fillStyle = ( isBlue = !isBlue ) ? '#99c0ff' : '#f1f9ff';
        contextBg.fill();
    }
  
    // 绘制伞骨
    for( var i = 0; i < step; i ++ ) {
        var  rad = ( 2 * Math.PI / 360 ) * ( 360 / step ) * i;
     
        var x = r + Math.sin( rad ) * r ;
        var y = r + Math.cos( rad ) * r ;
        contextBg.moveTo( r, r );
        contextBg.lineTo( x, y );
        //  输出项目文字
        var text = $( '<div class="text">' );
        text.text( cfg.data[i][0] );
        text.css( 'transition', 'all .5s ' + i * 0.1 + 's' );
    
        if ( x > w / 2 ) {
            text.css( 'left', x / 2 + 5 );
        } else {
            text.css( 'right', ( w - x ) / 2 + 5 );
        }
    
        if ( y > h / 2 ) {
            text.css( 'top', y / 2 + 5 );
        } else {
            text.css( 'bottom', ( h - y ) / 2 + 5 );
        }
        if ( cfg.data[i][2] ) {
            text.css( 'color', cfg.data[i][2] )
        }
        text.css( 'opacity', 0 );
    
        component.append( text) ;
    }
    contextBg.strokeStyle = '#e0e0e0';
    contextBg.stroke();
  

    // 绘制数据层
    // 加入一个画布（数据层）
    var canvasData = document.createElement( "canvas" );
    var contextData = canvasData.getContext( '2d' );
    canvasData.width  = contextData.width = w;
    canvasData.height = contextData.height = h;
    component.append( canvasData );
  
    contextData.strokeStyle = '#f00';
  
    var draw = function( per ){

        // 防止在火狐下出现线条紊乱 所以必要的清空当前路径栈
        contextData.beginPath();

        if ( per <= 1 ) {
            component.find( '.text' ).css( 'opacity', 0 );
        }
        if ( per >= 1 ) {
            component.find( '.text' ).css( 'opacity', 1 );
        }
        
        contextData.clearRect( 0, 0, w, h );
  
        // 输出数据的折线 ( 在火狐上出现)
        for ( var i = 0; i < step; i ++ ) {
            var rad = ( 2 * Math.PI / 360 ) * ( 360 / step ) * i;
            
            var rate  = cfg.data[i][1] * per;
            
            var x = r + Math.sin( rad ) * r * rate;
            var y = r + Math.cos( rad ) * r * rate;

            contextData.lineTo( x, y );
        }
        contextData.closePath();
        contextData.stroke();
  
        //  输出数据的点
        contextData.fillStyle = '#ff7676';
        for ( var i = 0; i < step; i ++ ) {
            var rad = ( 2 * Math.PI / 360 ) * ( 360 / step ) * i;
            
            var rate  = cfg.data[i][1] * per ;
    
            var x = r + Math.sin( rad ) * r * rate;
            var y = r + Math.cos( rad ) * r * rate ;
    
            contextData.beginPath();
            contextData.arc( x, y, 5, 0, 2 * Math.PI );
            contextData.fill();
            contextData.closePath();
        }
    };
    
    // 雷达图生长动画
    component.on( "afterLoad", function () {
        var step = 0;
        for( i = 0; i < 100;i ++ ){
          setTimeout( function () {
                step += 0.01;
                draw( step );
          }, i * 10 + 500 );
        }
    });

    // 雷达图退场动画
    component.on( "onLeave", function () {
        var step = 1;
        for( i = 0; i < 100; i ++ ){
          setTimeout( function () {
              step -= 0.01;
              draw( step );
          }, i * 10 );
        }
    });
      
    return component;
};